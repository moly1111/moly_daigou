...
- **发货清单**：待发货按用户聚合，按订单逐条展示地址；支持勾选一个或多个商品项发货（部分发货）
- **用户管理**：创建用户、封禁/解封、编辑、删除；支持多地址维护（默认地址）
- **仓储可视化**：按商品/规格查看库存；**趋势**：近 1/7/30 天销量 Top 10 与柱状图
- **数据库**：只读查看所有表；验证密码后可双击单元格编辑数据
- 仪表盘、基础设置、版本管理

### 其他
- **聊天**：用户与管理员双向聊天（文字、图片、文件）；聊天附件改为鉴权访问（非公开静态直链）
...
| `PRIVATE_UPLOAD_FOLDER` | 否 | 私有上传目录（聊天附件等），默认 `instance/private_uploads` |
...
- **公开上传**：`static/uploads/`（商品图、支付截图、封面、二维码等）
- **私有上传**：`instance/private_uploads/`（聊天图片/文件，需登录且通过权限校验后访问）
